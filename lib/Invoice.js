...
Invoice.prototype._setXML = function(method, fn) {
  ...
  switch(method) {
    ...
    case "invoice.lines.delete":
      request.node("invoice_id").text(self.invoice_id);
      if(this.lines.length > 0) {
        // Only able to delete one line at a time.
        var line = request.node("line_id").text( this.lines[0].id );
      }
    break;

    case "invoice.lines.update":
      request.node("invoice_id").text(self.invoice_id);
      var lines = request.node("lines");
      if(this.lines.length > 0) {
        this.lines.forEach(function(a) {
 ...
 *
 * @param {Number} line line item to delete
 * @param {Function} fn
 * @api public
 */
Invoice.prototype.deleteLine = function(line, fn) {
  var self = this;

  self.lines = [ line ];

  this._setXML("invoice.lines.delete", function(xml) {
    self.freshbooks._get(xml, function(err, xml) {
      if(null !== err) {
        fn(err);
      } else if("ok" !== xml.get("//xmlns:response",self.freshbooks.ns).attr("status").value()) {
...
/**
 * Update line items in an existing invoice.
 *
 * @param {Number} lines Array of line items to update
 * @param {Function} fn
 * @api public
 */
Invoice.prototype.updateLines = function(lines, fn) {
  var self = this;

  self.lines = lines;

  this._setXML("invoice.lines.update", function(xml) {
    self.freshbooks._get(xml, function(err, xml) {
      if(null !== err) {
        fn(err);
      } else if("ok" !== xml.get("//xmlns:response",self.freshbooks.ns).attr("status").value()) {
        err = xml.get("//xmlns:error",self.freshbooks.ns).text();
        fn(new Error("CANNOT UPDATE LINE ITEM FOR INVOICE: " + err));
      } else {
        self.get(self.invoice_id, fn);
      }
    });
